...
import { useContactInfo } from '@/lib/ContactInfoContext';
...
export default function Contact() {
  ...
  function onSubmit(data: FormValues) {
    ...
    /* EmailJS service integratie - UITGESCHAKELD
     * Dit onderdeel is uitgeschakeld om alleen berichten naar het admin dashboard te sturen
     * en niet de email client te openen. Verwijder de commentaren om dit weer in te schakelen.
     * 
    // EmailJS service IDs
    ...
    */
    ...
    /* Email verzenden via EmailJS - UITGESCHAKELD
    emailjs.send(serviceId, templateId, templateParams, publicKey)
      ...
      .catch((err) => {
        ...
        try {
          const recipientEmail = "noreply@portfoliojorian.com"; // Vervang met je echte email
...
// Helper functie om social icons te renderen
function renderSocialIcon(link: { platform: string; icon?: string }) {
  // Standaard iconen voor veelgebruikte platforms
  if (link.platform.toLowerCase() === 'github' || link.icon?.toLowerCase() === 'github') {
    return (
  ...
  if (link.platform.toLowerCase() === 'linkedin' || link.icon?.toLowerCase() === 'linkedin') {
    return (
      <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
  ...
  if (link.platform.toLowerCase() === 'twitter' || link.icon?.toLowerCase() === 'twitter') {
    return (
      <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
  ...
  if (link.platform.toLowerCase() === 'instagram' || link.icon?.toLowerCase() === 'instagram') {
    return (
      <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
  ...
  // Standaard icoon voor andere platforms
  return (
    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      <path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10zm0-7v-6m0 0L9 12m3-3l3 3"></path>
    </svg>
  );
}
